...
public class GuildMineGUI extends BaseGUI {
    ...
    public void build() {
        ...
        inventoryBuilder
                ...
                .item(2, 3, new ItemBuilder().material(Material.PAINTING).displayName("&f宗门公告").lores().colored().build())
        ...
        if (guild.isOwner(guildPlayer)) {
            inventoryBuilder
                    .item(45, new ItemBuilder().material(Material.ENDER_PORTAL_FRAME).displayName("&f管理宗门").colored().build())
                    .item(53, new ItemBuilder().material(Material.IRON_DOOR).displayName("&c解散宗门").colored().build(), new ItemListener() {
                        @Override
                        public void onClicked(InventoryClickEvent event) {
                            close();
                            JulyMessage.sendColoredMessage(bukkitPlayer, "&c[!!!操作不可逆] 如果要解散宗门, 请在聊天栏输入并发送: &econfirm");
